...
def scanline_convert(polygons, i, screen, zbuffer ):
    ...
    color = [ 29 * i % 256, 11 * i % 256, 209 * i % 256]
    ...
    y = int(y_bottom)
    while y < int(y_middle):
        y += 1
        draw_line(int(x0), int(y), int(z0), int(x1), int(y), int(z1), screen, zbuffer, color)
        x0 += (x_top - x_bottom) / (y_top - y_bottom)
        z0 += (z_top - z_bottom) / (y_top - y_bottom)
        x1 += (x_middle - x_bottom) / (y_middle - y_bottom)
        z1 += (z_middle - z_bottom) / (y_middle - y_bottom)
    ...
    while y < int(y_top):
        ...
        x0 += (x_top - x_bottom) / (y_top - y_bottom)
        z0 += (z_top - z_bottom) / (y_top - y_bottom)
        x1 += (x_top - x_middle) / (y_top - y_middle)
        z1 += (z_top - z_middle) / (y_top - y_middle)
